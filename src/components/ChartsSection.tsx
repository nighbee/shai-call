...
export const ChartsSection = ({ data }: ChartsSectionProps) => {
  ...
  const managerPerformance = Object.entries(
    data.reduce((acc: Record<string, { calls: number; avgRating: number; totalRating: number }>, call) => {
      const manager = call['Man name'];
      if (!acc[manager]) {
        acc[manager] = { calls: 0, avgRating: 0, totalRating: 0 };
      }
      acc[manager].calls += 1;
      acc[manager].totalRating += call['Overall Rating'];
